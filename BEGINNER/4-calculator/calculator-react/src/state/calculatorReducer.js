  ...
  switch (action.type) {
    ...
    case ACTIONS.BACKSPACE: {
      ...
      if (current.length <= 1 || (current.length === 2 && current.startsWith('-'))) {
        current = '0'
        return computeExpr({ ...state, current, overwrite: true })
      }
    ...
    case ACTIONS.MC: return { ...state, memory: 0 }
    case ACTIONS.MR: return computeExpr({ ...state, current: String(state.memory), overwrite: true })
    case ACTIONS.MPLUS: return { ...state, memory: state.memory + Number(state.current) }
    case ACTIONS.MMINUS: return { ...state, memory: state.memory - Number(state.current) }
    case ACTIONS.REPLAY: {
