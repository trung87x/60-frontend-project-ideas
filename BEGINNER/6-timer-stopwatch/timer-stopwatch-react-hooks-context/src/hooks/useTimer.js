...
export default function useTimer(defaultMinutes=1, defaultSeconds=30){
  ...
  const [seconds,setSeconds]=React.useState(defaultSeconds)
  const [running,setRunning]=React.useState(false)
  const [left,setLeft]=React.useState((defaultMinutes*60+defaultSeconds)*1000)
  const endAtRef=React.useRef(null)
  const rafRef=React.useRef(null)

  const totalMs=React.useMemo(()=> (clamp(minutes,0,99)*60 + clamp(seconds,0,59))*1000, [minutes,seconds])

  React.useEffect(()=>{ if(!running) setLeft(totalMs) },[totalMs,running])
