...
export default async function Home(){
  const items = await apiFetch('/posts?limit=4', { tags: ['posts'] }).catch(()=>[])
  return (
    <div className="space-y-8">
      <header className="text-center">
        <h1 className="text-3xl font-bold">Blog Platform (Next.js)</h1>
        <p className="opacity-80">Next.js Frontend + External Backend API</p>
      </header>
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-semibold">Latest Posts</h2>
          <Link href="/posts" className="text-sm underline">See all</Link>
        </div>
        <ul className="grid md:grid-cols-2 gap-3">
          {Array.isArray(items) && items.map((p:any)=>(
            <li key={(p.id||p.slug)} className="rounded-xl p-4 border border-white/10">
              ...
              <Link className="text-sm underline" href={`/posts/${(p.slug || p.id)}`}>View</Link>
            </li>
          ))}
        </ul>
      </section>
