...
export default async function Home(){
  const items = await apiFetch('/projects?limit=4', { tags: ['projects'] }).catch(()=>[])
  return (
    <div className="space-y-8">
      <header className="text-center">
        <h1 className="text-3xl font-bold">Project Management Tool</h1>
        ...
        <ul className="grid md:grid-cols-2 gap-3">
          {Array.isArray(items) && items.map((p:any)=>(
            <li key={(p.id||p.slug)} className="rounded-xl p-4 border border-white/10">
              ...
              <Link className="text-sm underline" href={`/projects/${(p.slug || p.id)}`}>View</Link>
            </li>
          ))}
        </ul>
      </section>
