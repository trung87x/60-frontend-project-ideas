...
export default async function Home(){
  const items = await apiFetch('/courses?limit=4', { tags: ['courses'] }).catch(()=>[])
  return (
    <div className="space-y-8">
      <header className="text-center">
        <h1 className="text-3xl font-bold">Learning Platform (LMS)</h1>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <Link href="/courses" className="text-sm underline">See all</Link>
        </div>
        <ul className="grid md:grid-cols-2 gap-3">
          {Array.isArray(items) && items.map((p:any)=>(
            <li key={(p.id||p.slug)} className="rounded-xl p-4 border border-white/10">
              ...
              <Link className="text-sm underline" href={`/courses/${(p.slug || p.id)}`}>View</Link>
            </li>
          ))}
        </ul>
      </section>
