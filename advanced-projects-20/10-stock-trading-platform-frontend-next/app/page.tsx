...
export default async function Home(){
  const items = await apiFetch('/symbols?limit=4', { tags: ['symbols'] }).catch(()=>[])
  return (
    <div className="space-y-8">
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-semibold">Latest Symbols</h2>
          <Link href="/symbols" className="text-sm underline">See all</Link>
        </div>
        <ul className="grid md:grid-cols-2 gap-3">
          {Array.isArray(items) && items.map((p:any)=>(
            <li key={(p.id||p.slug)} className="rounded-xl p-4 border border-white/10">
              ...
              <Link className="text-sm underline" href={`/symbols/${(p.slug || p.id)}`}>View</Link>
            </li>
          ))}
        </ul>
      </section>
