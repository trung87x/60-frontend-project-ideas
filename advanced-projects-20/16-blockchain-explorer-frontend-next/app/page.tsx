...
export default async function Home(){
  const items = await apiFetch('/blocks?limit=4', { tags: ['blocks'] }).catch(()=>[])
  return (
    <div className="space-y-8">
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <Link href="/blocks" className="text-sm underline">See all</Link>
        </div>
        <ul className="grid md:grid-cols-2 gap-3">
          {Array.isArray(items) && items.map((p:any)=>(
            <li key={(p.id||p.slug)} className="rounded-xl p-4 border border-white/10">
              ...
              <Link className="text-sm underline" href={`/blocks/${(p.slug || p.id)}`}>View</Link>
            </li>
          ))}
        </ul>
      </section>
